...
`./internal/... .` is used instead of `./...` because a stray GOPATH-shaped `pkg/` dir at the repo root makes `./...` fail with `pattern ./...: directory pkg/mod/... outside main module`. Integration tests run the real `files/migrations` in a per-package Postgres schema (`test_user`, `test_push`, `test_keys`) via `internal/testdb`, so packages are isolated and can run their tests in parallel. If you hit a failure, reset a package's test schema: `DROP SCHEMA test_<pkg> CASCADE`.
